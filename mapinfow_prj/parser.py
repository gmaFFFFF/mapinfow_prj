...
class MapinfowPrj:
    ...
    def _описать_грамматику(self):
        ...
        self._проекция = ~FollowedBy(self._афин_границы) + self._разделитель_обяз - \
                         self._единицы_изм - self._проекция_параметры
        ...
        self._датум_польз = (Literal("9999").suppress() ^ Literal("999").suppress()) - self._разделитель_обяз - \
                            (self._датум_польз3 ^
                             (self._датум_польз7 - FollowedBy(Opt(self._проекция) - self._афин_границы).suppress()))
